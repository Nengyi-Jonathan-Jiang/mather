...
inputEl.onkeydown = (e) => {
    if(e.key === 'Tab') {
        ...
        if(e.shiftKey) {
            // const start = inputEl.selectionStart;
            // const end = inputEl.selectionEnd;
            // let prefix = inputEl.value.substring(0, start);
            // const cut_length = (
            //     prefix.endsWith('    ') ? 4 :
            //     prefix.endsWith('   ') ? 3 :
            //     prefix.endsWith('  ') ? 2 :
            ...
            //     0
            // );
            // prefix = prefix.substring(0, prefix.length - cut_length)
            // inputEl.value = prefix + inputEl.value.substring(end);
            // inputEl.selectionStart = inputEl.selectionEnd = start - cut_length;
            ...
            if(cut_length > 0) {
                inputEl.selectionStart -= cut_length;
                inputEl.selectionEnd = start;
                document.execCommand('delete');
                inputEl.selectionStart = start - cut_length;
                inputEl.selectionEnd = end - cut_length;
            }
        }
        else {
...
setTimeout(()=>recalculate_sizes(displayEl), 0);